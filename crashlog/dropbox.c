...
int manage_duplicate_dropbox_events(struct inotify_event *event)
{
    ...
    if ( (event->mask & IN_MOVED_TO) && (previous_event_cookie != 0)
        ...
        /*
         * Fetch the timestamp from the original log filename and
         * write it in infoevent as a human readable date
         */
        if (extract_dropbox_timestamp(previous_filename, timestamp) == -1) {
            return -1;
        }
         ...
         */
        create_infoevent(info_filename, previous_filename, event->name, timestamp);
...
int process_lost_event(struct watch_entry __attribute__((unused)) *entry, struct inotify_event *event) {
    ...
    key = raise_event(CRASHEVENT, lostevent, lostevent_subtype, destion);
    LOGE("%-8s%-22s%-20s%s %s\n", CRASHEVENT, key, get_current_time_long(0), lostevent, destion);
    free(key);
    return 0;
}
